    ...
    for(;;)
    {
        ...
        switch(choice)
        {
                    ...
                    else
                    {
                        ...
                        else    //登录成功
                        {
                            ...
                            if(readthreadnumber == 0)
                            {
                                ...
                                readTask.detach();  //底层调用了pthread_detach,设置成分离线程，内核中PCB自动回收
                                readthreadnumber++;
                            }
            ...
            case 2:     //register业务
            {
                ...
                char pwd[50] = {0};
                cout << "username:";
                cin.getline(name,50);   //如果用cin 或 scanf遇到空格回车非法字符都会结束输入
                cout << "userpassword:";
                cin.getline(pwd,50);
                ...
                else
                {
                    ...
                    {
                        json responsejs = json::parse(buffer);  //网络接收的原始数据，反序列化成json对象
...
void groupchat(int clientfd,string str)
{
    ...
    int groupid = atoi(str.substr(0,idx).c_str());  //atoi把字符串转成一个整数,atoi处理的是char *,所以要加.c_str()
    string message = str.substr(idx + 1,str.size() - idx);
